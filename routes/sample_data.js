...
router.post("/addRisk", function(request, response, next) {
    ...
    var query = `
        INSERT INTO risks 
        (potential_Risk, Policy_Requirement, Assignee, Category, Department, Application_Name,Entities) 
        VALUES ("${potential_Risk}", "${policyRequirement}", "${assignee}", "${category}", "${department}", "${applicationName}","${Entities}")
    `;
    ...
    database.query(query, function(error, data) {
        if (error) {
            ...
            response.redirect("/sample_data"); // Adjust the redirect URL accordingly
        }
    });
});
 
router.get('/deleteRisk/:id', function(request, response, next){

	var id = request.params.id; 

	var query = `
	DELETE FROM risks WHERE id = "${id}"
	`;
